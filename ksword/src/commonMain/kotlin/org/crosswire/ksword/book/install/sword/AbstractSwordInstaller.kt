        ...
        if (!loaded) {
            ...
            TarGzExpander().handleTarGzContent(catalogFile) { name, content ->
                ...
                if (sbmd.isSupported) {
                    ...
                    Log.d("Skipping unsupported book: ${sbmd.initials} ${sbmd.getProperty(SwordBookMetaData.KEY_MOD_DRV)} ${sbmd.getProperty(BookMetaData.KEY_VERSIFICATION)} ${sbmd.getProperty(BookMetaData.KEY_SOURCE_TYPE)}")
                }
            }
